...
import { JobFilesBoxComponent } from '../../jobs-management/job-files-box/job-files-box.component';
...
export class PartnersComponent {
  ...
  ngOnInit() {
    ...
    this.dataSource.loadPartners(1, 10, '','partnerNumber asc');
  ...
  create() {
      console.log(this.referentsIsp);
      const partner = new Partner();
     const listReferents = this.referentsIsp;
      const dlg = this.dialog.open(PartnerDetailsBoxComponent, { data: { partner, isNew: true, listReferents, readonly: false },disableClose: true});
  ...
  edit(partner: Partner) {
    ...
    const dlg = this.dialog.open(PartnerDetailsBoxComponent,
      { data: { partner, isNew: false, listReferents,readonly: false },disableClose: true});
    dlg.beforeClosed().subscribe(res => {
        if (res) {
          console.log(res);
  ...
  openFileBox(partner: Partner) {
    ...
    const dlg = this.dialog.open(JobFilesBoxComponent, {
      data: { documentDetail, documents: docsList },
    });
