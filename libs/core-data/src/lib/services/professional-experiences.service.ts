...
export class ProfessionalExperiencesService {
  ...
  getAll(pageNumber: number, pageSize: number, beneficiaryId:string, filter:string= ''): Observable<ProfessionnalExperiences> {
    ...
    let url_ = this.baseUrl + '/api/professionalexperience';
    url_ = url_.replace(/[?&]$/, '');
        ...
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              ...
              return <Observable<ProfessionnalExperiences>>(<any>_observableCatch(e));
            }
          } else
            return <Observable<ProfessionnalExperiences>>(<any>_observableCatch(response_));
        })
      );
  }

  protected processGetAll(response: HttpResponseBase): Observable<ProfessionnalExperiences> {
    ...
    if (status === 200) {
      return servicesLib.blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          result200 = ProfessionnalExperiences.fromJS(resultData200);
    ...
    return _observableOf<ProfessionnalExperiences>(<any>null);
  }

  insert(command: ProfessionnalExperience): Observable<void> {
    let url_ = this.baseUrl + "/api/professionalexperience";
    url_ = url_.replace(/[?&]$/, "");
...
update(command: ProfessionnalExperience): Observable<void> {
  let url_ = this.baseUrl + "/api/professionalexperience";
  url_ = url_.replace(/[?&]$/, "");
...
delete(id: number): Observable<void> {
    console.log(id);
  let url_ = this.baseUrl + "/api/professionalexperience/{id}";
