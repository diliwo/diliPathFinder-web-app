...
export class TrainingFieldService {
...
getAll(pageNumber: number, pageSize: number, filter:string= '', orderBy:string=""): Observable<TrainingFields> {
      ...
      _observableCatch((response_: any) => {
        if (response_ instanceof HttpResponseBase) {
          try {
            ...
            return <Observable<TrainingFields>>(<any>_observableCatch(e));
          }
        } else
          return <Observable<TrainingFields>>(<any>_observableCatch(response_));
      })
    );
}

protected processgetAll(response: HttpResponseBase): Observable<TrainingFields> {
  ...
  if (status === 200) {
    return servicesLib.blobToText(responseBlob).pipe(
      _observableMergeMap((_responseText) => {
        ...
        result200 = TrainingFields.fromJS(resultData200);
  ...
  return _observableOf<TrainingFields>(<any>null);
}
}
