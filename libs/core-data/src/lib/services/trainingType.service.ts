...
import { TrainingType, TrainingTypes } from '@frontend/api-interface';
...
getAll(pageNumber: number, pageSize: number, filter:string= '', orderBy:string=""): Observable<TrainingTypes> {
      ...
      _observableCatch((response_: any) => {
        if (response_ instanceof HttpResponseBase) {
          try {
            ...
            return <Observable<TrainingTypes>>(<any>_observableCatch(e));
          }
        } else
          return <Observable<TrainingTypes>>(<any>_observableCatch(response_));
      })
    );
}

protected processgetAll(response: HttpResponseBase): Observable<TrainingTypes> {
  ...
  if (status === 200) {
    return servicesLib.blobToText(responseBlob).pipe(
      _observableMergeMap((_responseText) => {
        ...
        result200 = TrainingTypes.fromJS(resultData200);
  ...
  return _observableOf<TrainingTypes>(<any>null);
}
