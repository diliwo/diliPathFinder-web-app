...
import { Referent, ReferentListVm, UpsertReferenceCommand, User, Users } from '@frontend/api-interface';
...
export class ReferentService {
  ...
  getAll(
    ...
    pageSize: number,
    filter:string= '',
    orderBy:string=''): Observable<ReferentListVm> {
    ...
    let url_ = this.baseUrl + "/api/referents";
    url_ = url_.replace(/[?&]$/, "");
    ...
    return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
        ...
        if (response_ instanceof HttpResponseBase) {
            try {
                ...
                return <Observable<ReferentListVm>><any>_observableThrow(e);
            }
        } else
            return <Observable<ReferentListVm>><any>_observableThrow(response_);
    }));
  }

  protected processGetAll(response: HttpResponseBase): Observable<ReferentListVm> {
    ...
    if (status === 200) {
        ...
        result200 = ReferentListVm.fromJS(resultData200);
    ...
    return _observableOf<ReferentListVm>(<any>null);
  }

  upsert(command: UpsertReferenceCommand): Observable<void> {
    let url_ = this.baseUrl + "/api/referents";
    url_ = url_.replace(/[?&]$/, "");
...
delete(id: number): Observable<void> {
  let url_ = this.baseUrl + "/api/referents/{id}";
...
getReferentBySearch(text : string): Observable<Users> {
  let url_ = this.baseUrl + '/api/referents/searchtext/{text}';
  ...
  return this.http
    ...
    .pipe(
      _observableMergeMap((response_: any) => {
        return this.processGeReferentsBySearch(response_);
      })
    )
    .pipe(
      _observableCatch((response_: any) => {
        if (response_ instanceof HttpResponseBase) {
          try {
            return this.processGeReferentsBySearch(<any>response_);
          } catch (e) {
...
protected processGeReferentsBySearch(
