...
export class FormationsService {
  ...
  getAll(pageNumber: number, pageSize: number, filter:string= '',orderBy:string=""): Observable<Formations> {
    ...
    let url_ = this.baseUrl + '/api/formations';
    url_ = url_.replace(/[?&]$/, '');
        ...
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              ...
              return <Observable<Formations>>(<any>_observableCatch(e));
            }
          } else
            return <Observable<Formations>>(<any>_observableCatch(response_));
        })
      );
  }

  protected processGetAll(response: HttpResponseBase): Observable<Formations> {
    ...
    if (status === 200) {
      return servicesLib.blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          result200 = Formations.fromJS(resultData200);
    ...
    return _observableOf<Formations>(<any>null);
  }

  insert(command: Formation): Observable<void> {
    let url_ = this.baseUrl + "/api/formations";
    url_ = url_.replace(/[?&]$/, "");
...
update(command: Formation): Observable<void> {
  let url_ = this.baseUrl + "/api/formations";
  url_ = url_.replace(/[?&]$/, "");
...
delete(id: number): Observable<void> {
    console.log(id);
  let url_ = this.baseUrl + "/api/formations/{id}";
