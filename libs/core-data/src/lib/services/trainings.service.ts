...
export class TrainingsService {
  ...
  getAll(pageNumber: number, pageSize: number, filter:string= '',orderBy:string=""): Observable<Trainings> {
    ...
    let url_ = this.baseUrl + '/trainings';
    url_ = url_.replace(/[?&]$/, '');
        ...
        _observableCatch((response_: any) => {
          if (response_ instanceof HttpResponseBase) {
            try {
              ...
              return <Observable<Trainings>>(<any>_observableCatch(e));
            }
          } else
            return <Observable<Trainings>>(<any>_observableCatch(response_));
        })
      );
  }

  protected processGetAll(response: HttpResponseBase): Observable<Trainings> {
    ...
    if (status === 200) {
      return servicesLib.blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          result200 = Trainings.fromJS(resultData200);
    ...
    return _observableOf<Trainings>(<any>null);
  }

  insert(command: Training): Observable<void> {
    let url_ = this.baseUrl + "/trainings";
    url_ = url_.replace(/[?&]$/, "");
...
update(command: Training): Observable<void> {
  let url_ = this.baseUrl + "/trainings";
  url_ = url_.replace(/[?&]$/, "");
...
delete(id: number): Observable<void> {
    console.log(id);
  let url_ = this.baseUrl + "/trainings/{id}";
